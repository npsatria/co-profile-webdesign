//#region node_modules/typeit/dist/index.es.js
...
var calculateDelay = (delayArg) => {
	if (!isArray(delayArg)) delayArg = [delayArg / 2, delayArg / 2];
	return delayArg;
};
...
function walkElementNodes(element, shouldReverse = false, shouldIncludeCursor = false) {
	...
	let walker = document.createTreeWalker(element, NodeFilter.SHOW_ALL, { acceptNode: (node) => {
...
var cleanUpSkipped = ({ index, newIndex, queueItems, cleanUp }) => {
	for (let i = index + 1; i < newIndex + 1; i++) cleanUp(queueItems[i][0]);
};
var isNumber = (value) => Number.isInteger(value);
var countStepsToSelector = ({ queueItems, selector, cursorPosition, to }) => {
...
var setCursorAnimation = ({ cursor, frames, options }) => {
...
var rebuildCursorAnimation = ({ cursor, options, cursorOptions }) => {
...
var execute = (queueItem) => queueItem.func?.call(null);
var fireItem = async ({ index, queueItems, wait, cursor, cursorOptions }) => {
...
var isBodyElement = (node) => /body/i.test(node?.tagName);
...
var removeNode = (node, rootElement) => {
	...
	(nodeParent.childNodes.length > 1 || nodeParent.isSameNode(rootElement) ? node : nodeParent).remove();
};
...
var setCursorStyles = (id, element) => {
	let cursorSelector = `${`[${DATA_ATTRIBUTE}='${id}']`} .${CURSOR_CLASS}`;
	let computedStyles = getComputedStyle(element);
	appendStyleBlock(`${cursorSelector} { display: inline-block; width: 0; ${Object.entries(cursorFontStyles).reduce((accumulator, [item, value]) => {
...
var TypeIt = class {
	...
	*/
	...
	type(string, actionOpts = {}) {
		...
		let charsAsQueueItems = maybeChunkStringAsHtml(string, this.opts.html).map((char) => {
			return {
				func: () => this.#type(char),
				char,
	...
	pause(milliseconds, actionOpts = {}) {
		return this.#queueAndReturn({ delay: handleFunctionalArg(milliseconds) }, actionOpts);
	}
	...
	*/
	...
	#generateTemporaryOptionQueueItems(newOptions = {}) {
		return [{ func: () => this.#updateOptions(newOptions) }, { func: () => this.#updateOptions(this.opts) }];
	}
